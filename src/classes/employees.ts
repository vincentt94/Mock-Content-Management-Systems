...
class Employee {
    ...
    static async addEmployee(firstName: string, lastName: string, roleId: number, managerId: number) {
    ...
    static async updateEmployeeRoles(employeeId: number, newRoleId: number) {
        const query = `
        UPDATE employees
        SET role_id = $1
        WHERE id = $2
